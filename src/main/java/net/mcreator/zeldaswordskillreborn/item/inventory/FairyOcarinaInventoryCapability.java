...
public class FairyOcarinaInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == ZeldaSwordSkillRebornModItems.FAIRY_OCARINA.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(13) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != ZeldaSwordSkillRebornModItems.FAIRY_OCARINA.get();
			}
