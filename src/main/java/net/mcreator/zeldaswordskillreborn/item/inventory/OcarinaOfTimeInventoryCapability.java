...
public class OcarinaOfTimeInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == ZeldaSwordSkillRebornModItems.OCARINA_OF_TIME.get()) {
			if (Minecraft.getInstance().screen instanceof OcarinaGuiScreen) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(13) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != ZeldaSwordSkillRebornModItems.OCARINA_OF_TIME.get();
			}
