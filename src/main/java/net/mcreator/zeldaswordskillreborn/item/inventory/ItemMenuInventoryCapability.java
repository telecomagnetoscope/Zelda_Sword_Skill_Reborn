...
public class ItemMenuInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == ZeldaSwordSkillRebornModItems.ITEM_MENU.get()) {
			if (Minecraft.getInstance().screen instanceof ItemGuiScreen) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(34) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != ZeldaSwordSkillRebornModItems.ITEM_MENU.get();
			}
